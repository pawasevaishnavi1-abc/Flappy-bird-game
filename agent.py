...
class Agent:
    ...
    def run(self, is_training=True, render=False):
        ...
        if is_training:
            ...
            # best policy load
            # map_location=device avoids a crash if the model was saved
            # from a different device (e.g. trained on cuda/mps, run on cpu)
            ...
            for episode in itertools.count():
                ...
                while not terminated and episode_reward < self.reward_threshold:
                    ...
                    if is_training:
                        ...
                        # Train on every step (once we have enough samples),
                        # not just once per episode.
                        if len(memory) >= self.mini_batch_size:
                            mini_batch = memory.sample(self.mini_batch_size)
                            self.optimize(mini_batch, policy_dqn, target_dqn)

                            # sync the target network periodically, based on steps
                ...
                print(f"episode={episode+1} with total reward={episode_reward} & epsilon={epsilon if is_training else 'N/A'}")
